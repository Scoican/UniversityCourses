...
void consoleFindAll(Service *service){
	...
	int i;
	if (service->repository.vector.length == 0) {
		printf("No packages available!\n");
	}
	else {
		for (i = 0; i <service->repository.vector.length; i++) {
...
void consoleFilterByDestination(Service * service)
{
	...
	Package* packages = filterByDestination(&vector,service,destinantion);
	int i;
	if (length(&vector) == 0) {
		printf("No packages available!\n");
	}
...
void consoleFilterByPrice(Service * service)
{
	...
	double price = 0;
	readPrice(&price);
	Package* packages = filterByPrice(&vector, service, price);
	int i;
	if (length(&vector) == 0) {
		printf("No packages available!\n");
	}
...
int run() {
	...
	addPackage(&service, 1, "asd", "asd", "asd", 20);
	addPackage(&service, 2, "asd", "asdasd", "asd",  600);
	addPackage(&service, 3, "asd", "dsaasd", "asd", 19);
