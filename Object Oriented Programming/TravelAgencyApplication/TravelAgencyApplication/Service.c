...
int addPackage(Service* service, int id, char type[40], char destination[50], char date[11], double price) {
	...
	if (findPosition(&service->repository, &package) == -1) {
		return add(&service->repository, &package);
	}
	return -1;
}
...
Package* filterByDestination(Vector* newVector,Service* service,char destination[50]) {
	Package package1;
	int i;
	for (i = 0; i < length(&service->repository.vector); i++) {
		package1 = getPackage(&service->repository.vector,i);
		if (strcmp(getDestination(&package1), destination) == 0) {
			append(newVector, &package1);
		}
	}
	return newVector->packages;
}
